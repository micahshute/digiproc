RSpec.describe Digiproc::DigitalSignal do 
    ...
    let(:periodic_signal) do
        Digiproc::DigitalSignal.new_from_eqn(eqn: cos_pi_over_ten, size: 32)
    end

    let(:mod_signal) do
        Digiproc::DigitalSignal.new_from_eqn(eqn: n_mod_4, size: 32)
    end

    let(:short_signal_a) do
        Digiproc::DigitalSignal.new(data: [91, 23, 11, 53, 12, 82, 62])
    end

    let(:short_signal_b) do
        Digiproc::DigitalSignal.new(data: [5,4,2,3])
    end
    ...
    it "allows creation of a digital signal with an arary of data, #i can access any run or singular datapoints" do 
        ...
        ds = Digiproc::DigitalSignal.new(data: data)
        expect(ds).to be
    ...
    it "allows creation of a digital signal with an equation and size" do
        ds = Digiproc::DigitalSignal.new_from_eqn(eqn: cos_pi_over_ten, size: 20)
        expect(ds).to be
    ...
    it "allows creation of a digital signal with an array of equations and ranges" do
        ...
        ds = Digiproc::DigitalSignal.new_from_equations(eqns: eqns, ranges: ranges)
        ds2 = Digiproc::DigitalSignal.new_from_equations(eqns: eqns, ranges: [0..10, 11..15, 15..20])
    ...
    it "can return a data value, and returns a 0 outside the range" do
        ds = Digiproc::DigitalSignal.new(data: [1,2,3,4,5])
    ...
    it "#conv allows it to be convolved with another digital signal (or any convolvable object)" do
        ...
        ds1 = Digiproc::DigitalSignal.new(data: [1,2,3,4])
        ds2 = Digiproc::DigitalSignal.new(data: [1,2,3,4])
        expect(ds1.conv(ds2)).to eq([1, 4, 10, 20, 25, 24, 16])
    end

    it "#ds_conv convoles with a Digiproc::DigitalSignal and produces a new Digiproc::DigitalSignal" do
        expect(periodic_signal.ds_conv(mod_signal).is_a? Digiproc::DigitalSignal).to be(true)
        expect(periodic_signal.ds_conv(mod_signal).data).to eq(Digiproc::Functions.conv(periodic_signal.data, mod_signal.data))
    ...
    it "#fft.calculate can create an fft of itself, accessible via @fft" do
        expect(periodic_signal.fft.is_a?(Digiproc::FFT)).to eq(true)
        expect(periodic_signal.fft.calculate).to eq(Digiproc::FFT.new(time_data: periodic_signal.data).calculate)
        expect(periodic_signal.fft.fft.length).to eq(32)
    end

    it "#fft_magnitude can calculate magnitude of its values" do 
        expect(periodic_signal.fft_magnitude).to eq(Digiproc::FFT.new(time_data: periodic_signal.data).calculate.map{ |val| val.abs})
    end

    it "#fft_angle can calculate angle of its values" do
        expect(periodic_signal.fft_angle).to eq(Digiproc::FFT.new(time_data: periodic_signal.data).calculate.map{ |val| val.angle })
    end
    ...
    it "#ds_cross_correlation returns a Digiproc::DigitalSignal of its cross-correlation with an array or a correlatable" do
        expect(periodic_signal.ds_cross_correlation(mod_signal).is_a? Digiproc::DigitalSignal).to eq(true)
        expect(periodic_signal.ds_cross_correlation(mod_signal.data).is_a? Digiproc::DigitalSignal).to eq(true)
    ...
    it "xcorr is an alias for #cross_correlaion" do
        expect(periodic_signal.xcorr(mod_signal)).to eq(Digiproc::Functions.cross_correlation(periodic_signal.data, mod_signal.data))
        expect(mod_signal.xcorr(periodic_signal.data)).to eq(Digiproc::Functions.cross_correlation(mod_signal.data, periodic_signal.data))
    end

    it "ds_xcorr is an alias for ds_cross_correlation" do
        expect(periodic_signal.ds_xcorr(mod_signal).is_a? Digiproc::DigitalSignal).to eq(true)
        expect(periodic_signal.ds_xcorr(mod_signal).data).to eq(Digiproc::Functions.cross_correlation(periodic_signal.data, mod_signal.data))
        expect(mod_signal.ds_xcorr(periodic_signal.data).data).to eq(Digiproc::Functions.cross_correlation(mod_signal.data, periodic_signal.data))
    end
    ...
    it "#auto_correlation returns an array of the signal's auto correlation" do
        ...
        expect(periodic_signal.auto_correlation).to eq(Digiproc::Functions.cross_correlation(periodic_signal.data, periodic_signal.data))
    end

    it "#ds_auto_correlation returns a Digiproc::DigitalSignal of the signal's auto correlation" do
        expect(periodic_signal.ds_auto_correlation.is_a? Digiproc::DigitalSignal).to eq(true)
    ...
    it "#ds_acorr is an alias for ds_auto_correlation" do
        expect(periodic_signal.ds_acorr.is_a? Digiproc::DigitalSignal).to eq(true)
    ...
    it "#power_spectral_density can return its power spectral density (fft of autocorrelation) as an Digiproc::FFT class" do
        expect(periodic_signal.power_spectral_density.is_a? Digiproc::FFT).to eq(true)
    ...
    it "#psd is an alias for #power_spectral_density" do
        expect(periodic_signal.psd.is_a? Digiproc::FFT).to eq(true)
    ...
    it "#cross_spectral_density takes a signal and can return a Digiproc::FFT" do
        expect(periodic_signal.cross_spectral_density(short_signal_a.data).is_a? Digiproc::FFT).to eq(true)
    ...
    it "#csd is an alias for @cross_spectral_density" do
        expect(periodic_signal.csd(short_signal_a).is_a? Digiproc::FFT).to eq(true)
